...
ターゲットの `dependencies` に `MediaAgent` を追加する（推移的に `MediaAgentTools` と `MediaStore` も解決される）:
...
外部 API キーなしでも `save_image_url` / `save_video_reference` / `list_saved_media` の 3 コアツールと `create_chart` の計 4 ツールは常に提供される。

```swift
let kit = MediaToolKit(store: store)  // http は URLSession デフォルト
let tools = kit.tools                 // コアツール 3 + create_chart（計 4）
```
